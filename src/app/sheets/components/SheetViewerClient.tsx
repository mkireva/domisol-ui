...
const LyricsView = dynamic(() => import("./LyricsView").then(mod => ({ default: mod.LyricsView })), {
  loading: () => <div>Loading lyrics...</div>
});

const InfoCard = dynamic(() => import("./InfoCard").then(mod => ({ default: mod.InfoCard })), {
  loading: () => <div>Loading info...</div>
});

const SheetAudioPlayer = dynamic(() => import("./SheetAudioPlayer").then(mod => ({ default: mod.SheetAudioPlayer })), {
...
export function SheetViewerClient({
  ...
  const availableLanguages = useMemo(() => lyrics.map((l) => l.language), [lyrics]);

  const renderTabContent = useCallback((tab: string) => {
    switch (tab) {
      case "sheet":
        return initialSheetUrl && <MusicSheetDisplay initialSheet={initialSheetUrl} />;
      case "lyrics":
        return hasLyrics && (
          <LyricsView
            sheet={selectedSheet}
  ...
  return (
    ...
    >
      ...
      {TABS.map(({ id }) => (
        <TabsContent key={id} value={id} className={`mt-${id === 'sheet' ? '4' : '12'}`}>
